...
Figure \ref{fig:comparison-stst-baseline-dist} plots the two steady-state distributions of agents over their asset positions, cut off at bond holdings of $30$ for readability. The two distributions look very similar. Trivially, no mass of agents is admissible between the old and the new borrowing limit in the new steady state, so the density there is zero. Overall, the asset distribution seems to have been moved to the right. However, as in \textcite{gl2017}, there is also a more subtle but important change: the tighter borrowing limit actually compresses the distribution rather than just pushing it to the right. On the one hand, at the remaining admissible levels of debt and at low positive asset holdings, the density of the terminal state lies above the density of the initial state. On the other hand, less agents are located at high bond holdings in an environment of tight credit relative to before. Thus, the asset distribution is compressed. Incidentally, this results in a reduced wealth inequality as measured by the asset share of the top-$10\%$, see table \ref{tab:stst_comparison_baseline_limit_permanent}.

% What is more, table \ref{tab:stst_comparison_baseline_limit_permanent} reveals that $5.07\%$ of the population now choose to be directly at the borrowing limit, a slight increase from the initial $4.89\%$.

Figure \ref{fig:comparison-stst-baseline-policies-a} sheds light on why this compression takes place by comparing the average asset accumulation in each steady state.\footnote{Asset accumulation is the asset policy less the $45$ degree line, i.e.~less a hypothetical policy according to which assets remain the same for the next period along all asset holdings. The average is over productivity levels.} In the terminal steady state, the average household reduces debt holdings faster when being a borrower and draws down positive asset holdings faster than in the initial steady state. How are agents persuaded to behave in this new way? Consider first the borrowers. As the borrowing limit is now closer for all agents, and especially so for the ones in debt, households would like to reduce the possibility of reaching the debt limit in finite time. To achieve this, agents aim at reducing borrowing faster whenever given the chance to do so, e.g.~in the case of an above-average labour skill realisation. Thus, on average, they pay back debt in a higher pace than in the initial steady state.

All else equal, the asset-rich households would likewise aim for higher precautionary savings due to the borrowing limit being a more imminent threat. To understand why these households nevertheless decumulate assets faster rather than accumulate these quicker, consider table \ref{tab:stst_comparison_baseline_limit_permanent}. It reveals that the real interest rate is permanently lower in the terminal steady state than in the initial steady state. As these households are interest-rate sensitive -- recall from subsection \ref{sec:model-hh-constraints} that the asset-rich behave here like a permanent-income consumer -- the lower real interest rate induces the wealthy to substitute away from asset holdings and towards consumption. Figure \ref{fig:init-stst-dist-pol-c} reflects this. In an environment of tight credit, individuals at the lower end of the asset distribution consume less in order to pay back debt, as described above. In contrast, consumption for households at high asset levels is higher as the lower real interest rate induces them to do so.

This raises the question as to why the real interest rate is lower in the terminal steady state. Crucially, this drop is a direct consequence of the adjustments in the asset market: a reduction in the supply of liquidity and an increase in the demand for liquidity. As documented by \textcite{gl2017} and as is the case here, two forces contribute to these developments.  The first one is the deleveraging forced onto the households at and close to the initial borrowing limit. Prohibiting these agents to hold their initial levels of debt crucially prevents them to issue \Gls{iou}s. That is, the shock to the borrowing limit mechanically restricts the supply of \Gls{iou}s and thus the supply of assets which the productive households can invest in. This is the crucial aspect of the financial shock considered here: it not only affects borrowers but, through debtors' balance sheets, also the lenders.

The second force leading to adjustments in the asset market is the increased desire for precautionary savings by the unconstrained agents. Even these agents dread the now closer debt limit and would in principle like to save more. The resulting increase in demand for safe assets meets, however, the aforementioned decrease in supply of assets. Taken together, these changes call for a lower interest rate in order to achieve equilibrium in the asset market. Essentially, the reduced interest rate prompts some borrowers to supply some more assets and lenders to demand less assets.

Note that, as expected, aggregate consumption is identical across steady states as it is pinned down by the unchanged degree of substitutability among intermediate goods. Intuitively, lower interest rates (and taxes) enable consumption to remain unchanged in this model, even in an environment of tight credit.\footnote{Hence, the model can not speak to considerations about the long-term connection of credit availability and aggregate consumption.} Incidentally, the same level of aggregate consumption is more unevenly distributed in the long run.
...
How does the economy evolve \textit{during the transition} from the steady state from subsection \ref{sec:limit-stst-loose} to the one described in subsection \ref{sec:limit-stst-tight}? That is, how does the shock to the borrowing limit work through the households' balance sheets over time and then create, in general equilibrium, distributional and aggregate dynamics?
...
Figure \ref{fig:baseline-permanent-limit-agg} presents the responses of some key variables. For the sake of brevity, I refer the reader to figure \ref{fig:baseline-permanent-limit-agg-extra} in appendix \ref{sec-app:figures-limit-transition} for some supplementary responses. First, plot \ref{fig:baseline-permanent-limit-agg-phi} confirms that the borrowing limit completes most of its transition from $\phi_{ss} = -2.3485$ to $\phi_{ss}' = -2.1775$ within six model periods, as calibrated.
...
In $t=1$, when the shock is announced, the debt limit already contracts by almost half of its total change. As can be seen from the plots in figure \ref{fig:baseline-permanent-limit-agg}, on impact, debt and consumption fall, while the debt-to-\Gls{gdp} ratio and the real interest rate spike. The central bank immediately adjusts its target rate to the terminal rate (not shown) and reduces the nominal interest rate to zero. Figure \ref{fig:baseline-permanent-limit-agg-extra} further shows that total labour hours go down, wages, marginal costs and inflation drop and taxes as well as dividends surge. 

How do these impact responses come about? In the following, I draw on the aggregate and cross-sectional dimensions to describe the dynamics.
...
The first direct effect of the reduction in the debt limit is the deleveraging forced on the immediately constrained agents.

%In contrast to the model of \textcite{gl2017}, here negative real interest rates can arise naturally.
...
\newpage
\thispagestyle{plain}
\pagenumbering{roman}  % Turn page numbering to Roman
